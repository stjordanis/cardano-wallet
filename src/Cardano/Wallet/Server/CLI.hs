...
data WalletBackendParams = WalletBackendParams
    ...
    -- ^ Using push (readerT based, introducing coupling to node) mechanism (False),
    -- Or push mechanism (decoupling from the node) - True
    } deriving Show
...
walletBackendParamsParser = WalletBackendParams <$> enableMonitoringApiParser
  ...
  where
    ...
    applyBlockPullMechanismParser = switch $
                          long "pull-mechanism" <>
                          help "Enforces pull mechanism for block aquisition from the node. \
